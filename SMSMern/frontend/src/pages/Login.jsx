...
export default function Login() {
  ...
  // FIX: Dynamically determine the API URL
  // If the app is on Vercel, it uses the environment variable.
  // If you are on your computer, it defaults to your local server port (usually 5000 for Node).
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        else if (role === 'student' || role === 'parent') navigate('/student-dashboard')
        else navigate('/employee-dashboard')
      } else {
        setError(response.data.error || 'Login failed')
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-teal-600 to-sky-100 p-6">
      <div className="max-w-md w-full bg-white/90 backdrop-blur-md rounded-2xl shadow-lg p-8">
        ...
        <h2 className="text-xl font-medium my-4">Welcome back 👋</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="relative mb-6">
            ...
            <button
              type="button"
              onClick={() => setShowPassword((s) => !s)}
              className="absolute right-2 top-2 text-xs px-2 py-1 rounded-md bg-gray-100 hover:bg-gray-200"
            >
              {showPassword ? 'Hide' : 'Show'}
            </button>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-teal-600 text-white py-2 rounded-lg font-medium hover:bg-teal-700 disabled:opacity-70 transition-colors"
          >
        ...
        <div className="mt-4 text-xs text-gray-400 text-center">
          © {new Date().getFullYear()} Smart Curriculum
        </div>
      </div>
    </div>
  )
